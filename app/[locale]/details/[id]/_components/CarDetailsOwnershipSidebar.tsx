...
export default function CarDetailsOwnershipSidebar({ car }: CarDetailsOwnershipSidebarProps) {
  ...
  const [fuelCostInput, setFuelCostInput] = useState(car.specs.fuelType === 'electric' ? 0.18 : 1.65);
  ...
  const loanPaymentCalculated = useMemo(
    () => calculateLoanPayment(car.price, downpayment, interestRate, loanTerm),
    [car.price, downpayment, interestRate, loanTerm],
  );

  const energyCostCalculated = useMemo(
    () => calculateEnergyCost(monthlyMileage, car.specs.fuelEfficiency, car.specs.fuelType, fuelCostInput),
    [monthlyMileage, car.specs.fuelEfficiency, car.specs.fuelType, fuelCostInput],
  );

  const insuranceCalculated = useMemo(() => {
    if (driverProfile === 'youth') return Math.round(car.expenses.insurance * 1.75);
    if (driverProfile === 'veteran') return Math.round(car.expenses.insurance * 0.82);
    return car.expenses.insurance;
  }, [car.expenses.insurance, driverProfile]);

  const repairsCalculated = useMemo(() => {
    if (usageIntensity === 'gentle') return Math.round(car.expenses.repairs * 0.7);
    if (usageIntensity === 'sport') return Math.round(car.expenses.repairs * 1.45);
    return car.expenses.repairs;
  }, [car.expenses.repairs, usageIntensity]);

  const depreciationCalculated = car.expenses.depreciation;

  const totalMonthlyTCO = useMemo(
    () => calculateTotalMonthlyTCO(loanPaymentCalculated, repairsCalculated, insuranceCalculated, depreciationCalculated, energyCostCalculated),
    [loanPaymentCalculated, repairsCalculated, insuranceCalculated, depreciationCalculated, energyCostCalculated],
  );
      ...
      <div className="rounded-2xl border border-neutral-200 bg-white p-5 dark:border-neutral-800 dark:bg-neutral-900/60 space-y-6">
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-1.5">
            ...
            <input type="range" min="0" max={Math.round(car.price * 0.8)} step="500" value={downpayment}
