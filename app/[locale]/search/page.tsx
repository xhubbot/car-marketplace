...
export default function SearchPage() {
  ...
  const { loanProviders, isLoading: loanProvidersLoading } = useLoanProviders()
  const { insuranceProviders, isLoading: insuranceProvidersLoading } = useInsuranceProviders()

  const loanProviderOptions = useMemo(
    () => loanProviders.map((p) => ({ value: p.id, label: `${p.name} (${p.interestRateAnnual}%)` })),
    [loanProviders]
  )
  const insuranceProviderOptions = useMemo(
    () => insuranceProviders.map((p) => ({ value: p.id, label: p.name })),
    [insuranceProviders]
  )
  ...
  const filtersKey = JSON.stringify([
    mode, makeId, modelId, fuelTypeId, transmissionId, loanProviderId, insuranceProviderId,
    priceMin, priceMax, ownershipMin, ownershipMax, repairMin, repairMax, monthlyKm,
  ])
  ...
  useEffect(() => {
    ...
    if (loanProviderId) query.set('loanProviderId', String(loanProviderId))
    if (insuranceProviderId) query.set('insuranceProviderId', String(insuranceProviderId))
    if (repairMin) query.set('repairMin', repairMin)
    if (repairMax) query.set('repairMax', repairMax)
    if (mode === 'price') {
    ...
    mode, locale, page, monthlyKm, makeId, modelId, fuelTypeId, transmissionId,
    loanProviderId, insuranceProviderId, priceMin, priceMax, ownershipMin, ownershipMax, repairMin, repairMax,
  ])
  ...
  const selectedLoanProvider = useMemo(
    () => loanProviderOptions.find((p) => p.value === loanProviderId) ?? null,
    [loanProviderOptions, loanProviderId]
  )
  const selectedInsuranceProvider = useMemo(
    () => insuranceProviderOptions.find((p) => p.value === insuranceProviderId) ?? null,
    [insuranceProviderOptions, insuranceProviderId]
  )
    ...
    <div className="min-h-screen bg-[#faf9f6] text-neutral-900 dark:bg-neutral-950 dark:text-neutral-100 font-sans">
      ...
      <main className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
        <div className="mb-6 space-y-1">
          <h1 className="text-2xl font-extrabold tracking-tight sm:text-3xl">Search Cars</h1>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          ...
          <aside className="lg:col-span-4 space-y-4 lg:sticky lg:top-24">
            {/* Financing — only relevant to Ownership Cost mode, so it's hidden entirely in Sticker Price mode */}
            {mode === 'ownership' && (
              <div className="rounded-2xl border border-emerald-200 bg-emerald-50/40 p-5 dark:border-emerald-900/50 dark:bg-emerald-950/10 space-y-4 shadow-xs">
                <h2 className="flex items-center gap-1.5 text-xs font-bold uppercase tracking-wide text-emerald-700 dark:text-emerald-400">
                  <Sparkles className="h-3.5 w-3.5" />
                  Financing
                </h2>
                ...
                <div>
                  <label className="block text-xs font-bold uppercase text-neutral-500 mb-1">Loan Provider</label>
                  <Select
                    instanceId="search-loan-provider"
                    options={loanProviderOptions}
                    value={selectedLoanProvider}
                    onChange={(o) => setLoanProviderId(o?.value ?? null)}
                    isLoading={loanProvidersLoading}
                ...
                <div>
                  <label className="block text-xs font-bold uppercase text-neutral-500 mb-1">Insurance Provider</label>
                  <Select
                    instanceId="search-insurance-provider"
                    options={insuranceProviderOptions}
                    value={selectedInsuranceProvider}
                    onChange={(o) => setInsuranceProviderId(o?.value ?? null)}
                    isLoading={insuranceProvidersLoading}
                    placeholder="Default provider…"
                    isClearable
                    styles={selectStyles}
                  />
                ...
                <div className="space-y-2 rounded-xl bg-emerald-100/60 p-3 dark:bg-emerald-950/20">
                  <div className="flex items-center gap-2">
                  ...
                  <div className="flex gap-1.5 text-[11px] text-emerald-800 dark:text-emerald-400">
                    ...
                    <span>Ownership cost = real loan + insurance + repair estimate + fuel/electricity at this mileage.</span>
                  </div>
                </div>

                <div>
                  <label className="block text-xs font-bold uppercase text-neutral-500 mb-1">Repair Cost Average (EUR/mo)</label>
                  <div className="flex items-center gap-2">
                    <input
                      type="number"
                      min="0"
...
function ListingCard({ listing, locale, mode }: { listing: SearchListingResult; locale: string; mode: SearchMode }) {
  const href = `/${locale}/details/${listing.id}`
  const primary =
    mode === 'price'
